//
// Created by gyyn on 2019/5/26.
//

#pragma once
#ifndef CGI_PROCESSPOOL_H
#define CGI_PROCESSPOOL_H
...
class processpool {
private:
    // 将构造函数设为私有的，因此我们只能通过后面的create静态函数来创建processpool实例
...
void processpool<T>::run_child() {
    ...
    while (!m_stop) {
        // 函数成功时返回就绪的文件描述符的个数				//ETIMEOUT110
        ...
        for (int i = 0; i < number; ++i) {
            ...
            if ((sockfd == pipefd) && (events[i].events & EPOLLIN)) {
                ...
                if (((ret < 0) && (errno != EAGAIN)) || ret == 0) {
                    ...
                    // 成功时返回一个新的连接socket，该socket唯一地标识了被接受的这个连接，服务器可通过读写该socket来与被接受连接对应的客户端通信
    ...
    users = nullptr;
    close(pipefd);
    // close(m_listenfd); 应该由m_listenfd的创建者来关闭这个文件描述符
    close(m_epollfd);
}
                            ...
                            case SIGCHLD: {
                                ...
                                while ((pid = waitpid(-1, &stat, WNOHANG)) > 0) {
                                    for (int i = 0; i < m_process_number; ++i) {
                                        ...
                                        if (m_sub_process[i].m_pipefd == pid) {
                                            printf("child %d join\n", i);
                                            close(m_sub_process[i].m_pipefd[0]);
